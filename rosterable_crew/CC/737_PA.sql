...
WHERE A.STAFF_NUM = B.STAFF_NUM
    ...
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='AC73' and q.expiry_dts>(TO_DATE(to_char(trunc(add_months(sysdate,&1), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') + 4 + 23/24 + 59/1440))
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='E73' and q.expiry_dts>(TO_DATE(to_char(trunc(add_months(sysdate,&1), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') + 4 + 23/24 + 59/1440))
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='K73' and q.expiry_dts>(TO_DATE(to_char(trunc(add_months(sysdate,&1), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') + 4 + 23/24 + 59/1440))
