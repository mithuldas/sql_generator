...
-- make sure that each crew has all the mandatory qualifications and expiry date is at least 05th of the month (i.e. 5 rosterable "qualification" days)    
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='AC32' and q.expiry_dts>(TO_DATE(to_char(trunc(add_months(sysdate,&1), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') + 4 + 23/24 + 59/1440))
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='E32' and q.expiry_dts>(TO_DATE(to_char(trunc(add_months(sysdate,&1), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') + 4 + 23/24 + 59/1440))
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='K32' and q.expiry_dts>(TO_DATE(to_char(trunc(add_months(sysdate,&1), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') + 4 + 23/24 + 59/1440))
