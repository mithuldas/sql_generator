...
and q.issued_dts < to_date((select dt from roster_str),'DDMONYY') and q.qual_cd IN (select qual_cd from qualifications_v where qual_type in ('LG','EQ','AN')) order by qual_cd, staff_num;