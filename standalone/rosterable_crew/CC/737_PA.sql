...
WHERE A.STAFF_NUM = B.STAFF_NUM
	...
	and exists (select 1 from crew_qualifications_v q where q.staff_num=a.staff_num and qual_cd='PA73' AND (
				(EXPIRY_DTS IS NULL)
    ...
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='AC73' and q.expiry_dts>(to_date((select dt from roster_str),'DDMONYY') + 4 + 23/24 + 59/1440))
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='E73' and q.expiry_dts>(to_date((select dt from roster_str),'DDMONYY') + 4 + 23/24 + 59/1440))
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='K73' and q.expiry_dts>(to_date((select dt from roster_str),'DDMONYY') + 4 + 23/24 + 59/1440))
