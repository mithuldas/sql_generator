...
WHERE A.STAFF_NUM = B.STAFF_NUM
    ...
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='ACS9' and q.expiry_dts>(to_date((select dt from roster_str),'DDMONYY') + 4 + 23/24 + 59/1440))
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='ES9' and q.expiry_dts>(to_date((select dt from roster_str),'DDMONYY') + 4 + 23/24 + 59/1440))
    and exists (select 1 from crew_qualifications_v q where a.staff_num=q.staff_num and qual_cd='KS9' and q.expiry_dts>(to_date((select dt from roster_str),'DDMONYY') + 4 + 23/24 + 59/1440))
