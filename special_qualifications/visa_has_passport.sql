...
and q.issued_dts < to_date(to_char(trunc(add_months(sysdate,&1), 'MON'),'YYYYMMDDHH24MI'),'YYYYMMDDHH24MI') and q.qual_cd in (select QUAL_CD from QUAL_TO_CTRY_MAP_V where qual_cd !='PARU') and exists
(select 1 from crew_qualifications_v q1 where q1.staff_num= a.staff_num and qual_cd='PARU') and passport_reference is not null
order by qual_cd, staff_num;
