...
and q.issued_dts < to_date(to_char(trunc(add_months(sysdate,&1), 'MON'),'YYYYMMDDHH24MI'),'YYYYMMDDHH24MI') and q.qual_cd IN (select qual_cd from qualifications_v where qual_type in ('LG','EQ','AN')) order by qual_cd, staff_num;