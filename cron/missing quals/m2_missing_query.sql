...
WHERE A.STAFF_NUM = B.STAFF_NUM	AND A.STAFF_NUM = C.STAFF_NUM AND A.STAFF_NUM = P.STAFF_NUM
	...
	AND A.STAFF_NUM IN (
		...
		GROUP BY STAFF_NUM)
	AND B.RANK_CD IN ('PA','BCA','EC','GM')	AND ((TO_DATE(to_char(trunc(add_months(sysdate,2), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') < P.EXP_DT OR P.EXP_DT IS NULL) AND TO_DATE(to_char(trunc(last_day(add_months(sysdate,2)),'DD'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') > P.EFF_DT)
	--QUAL
