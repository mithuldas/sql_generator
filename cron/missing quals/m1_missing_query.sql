...
WHERE A.STAFF_NUM = B.STAFF_NUM	AND A.STAFF_NUM = C.STAFF_NUM AND A.STAFF_NUM = P.STAFF_NUM
	...
	AND ((TO_DATE(to_char(trunc(add_months(sysdate,1), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') < B.EXP_DT OR B.EXP_DT IS NULL)	AND TO_DATE(to_char(trunc(last_day(add_months(sysdate,1)),'DD'),'YYYYMMDDHH24MI') > B.EFF_DT)
	AND ((TO_DATE(to_char(trunc(add_months(sysdate,1), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') < C.EXP_DT OR C.EXP_DT IS NULL) AND TO_DATE(to_char(trunc(last_day(add_months(sysdate,1)),'DD'),'YYYYMMDDHH24MI') > C.EFF_DT)
	AND A.STAFF_NUM IN (
		...
		OR (TO_DATE(to_char(trunc(last_day(add_months(sysdate,1)),'DD'),'YYYYMMDDHH24MI') BETWEEN EFF_DT AND NVL(EXP_DT, TO_DATE('31-DEC-2999', 'DD-MON-YYYY'))))
		GROUP BY STAFF_NUM)
	AND B.RANK_CD IN ('PA','BCA','EC','GM')	AND ((TO_DATE(to_char(trunc(add_months(sysdate,1), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') < P.EXP_DT OR P.EXP_DT IS NULL) AND TO_DATE(to_char(trunc(last_day(add_months(sysdate,1)),'DD'),'YYYYMMDDHH24MI') > P.EFF_DT)
	--QUAL
	AND (EXISTS (SELECT 1 FROM CREW_QUALIFICATIONS_V Q WHERE Q.STAFF_NUM=A.STAFF_NUM AND QUAL_CD IN ('MCH') AND (NVL(EXPIRY_DTS, TO_DATE('31-DEC-2073', 'DD-MON-YYYY')) 
			BETWEEN TO_DATE(to_char(trunc(add_months(sysdate,-2), 'MON'),'YYYYMMDDHH24MI'), 'YYYYMMDDHH24MI') AND TO_DATE(to_char(trunc(last_day(add_months(sysdate,1)),'DD'),'YYYYMMDDHH24MI'))) OR  
