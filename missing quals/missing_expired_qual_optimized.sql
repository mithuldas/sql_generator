...
WHERE A.STAFF_NUM = B.STAFF_NUM	AND A.STAFF_NUM = C.STAFF_NUM AND A.STAFF_NUM = P.STAFF_NUM
	...
	AND A.STAFF_NUM IN (
		...
		GROUP BY STAFF_NUM)
	AND B.RANK_CD IN ('PA','BCA','EC','GM')	AND ((TO_DATE('201502010000', 'YYYYMMDDHH24MI') < P.EXP_DT OR P.EXP_DT IS NULL) AND TO_DATE('201502280000', 'YYYYMMDDHH24MI') > P.EFF_DT)
	--QUAL
	AND (EXISTS (SELECT 1 FROM CREW_QUALIFICATIONS_V Q WHERE Q.STAFF_NUM=A.STAFF_NUM AND QUAL_CD IN ('MCH') AND (NVL(EXPIRY_DTS, TO_DATE('31-DEC-2073', 'DD-MON-YYYY')) 
			BETWEEN TO_DATE('201411010000', 'YYYYMMDDHH24MI') AND TO_DATE('201502280000', 'YYYYMMDDHH24MI'))) OR  
	...
	AND NOT EXISTS (SELECT 1 FROM CREW_QUALIFICATIONS_V Q WHERE Q.STAFF_NUM=A.STAFF_NUM AND QUAL_CD = 'MCH' AND ACTUALS_STATUS IN ('T', 'V')
                AND EXPIRY_DTS > TO_DATE ('28FEB15 2359', 'DDMONYY HH24MI')) 
  ORDER BY STAFF_N;
  
SELECT  DISTINCT A.STAFF_NUM  AS STAFF_N, A.PREFERRED_NAME,	A.FIRST_NAME, A.SURNAME, B.RANK_CD,	C.BASE,	P.RSRC_GRP_CD, 'MC' AS "QUAL"
FROM CREW_RANK_V B,	CREW_BASE_V C, CREW_RSRC_GRP_V P, CREW_V A
WHERE A.STAFF_NUM = B.STAFF_NUM	AND A.STAFF_NUM = C.STAFF_NUM AND A.STAFF_NUM = P.STAFF_NUM
	AND (A.TERM_DT >= TO_DATE('201502010000', 'YYYYMMDDHH24MI')	OR A.TERM_DT IS NULL)
	AND (A.RETR_DT >= TO_DATE('201502010000', 'YYYYMMDDHH24MI')	OR A.RETR_DT IS NULL)
